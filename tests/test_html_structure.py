...
def test_no_native_confirm_in_app_js():
    ...
    # Match `confirm(` but not `confirmModal(` and not `// confirm`
    matches = re.findall(r"(?<![a-zA-Z._])confirm\(", JS)
    # Allow uses inside the confirmModal function definition itself
    matches = [m for m in matches if "confirmModal" not in m]
    assert not [m for m in re.finditer(r"(?<![a-zA-Z._])confirm\(", JS)
