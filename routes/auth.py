...
def login():
    ...
    if not allowed:
        log.warning("login: rate-limited %s (retry in %ds)", ip, int(retry_in))
        return jsonify({
            "ok":    False,
...
def passkey_register_begin():
    ...
    if not allowed:
        log.warning("passkey-register: rate-limited %s (retry in %ds)", ip, int(retry_in))
        return jsonify({
            "ok":    False,
            "error": f"Too many attempts. Try again in {int(retry_in)} seconds.",
        }), 429
...
def passkey_auth_finish():
    ...
    if not allowed:
        log.warning("passkey-auth: rate-limited %s (retry in %ds)", ip, int(retry_in))
        return jsonify({
            "ok":    False,
            "error": f"Too many attempts. Try again in {int(retry_in)} seconds.",
        }), 429
    ...
    if not matching:
        log.info("passkey-auth: unknown credential id from %s", ip)
        return jsonify({"ok": False, "error": "Authentication failed"}), 401
    ...
    try:
        ...
        log.info("passkey-auth: verification failed from %s: %s", ip, e)
        return jsonify({"ok": False, "error": "Authentication failed"}), 401
