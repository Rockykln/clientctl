...
def list_displays():
    try:
        ...
        log.exception("display list failed")
        return jsonify({"ok": False, "error": "Could not list displays"}), 500
...
def set_brightness(display_id: str):
    ...
    try:
        ...
        # Validation failure on display_id — known shape, fixed message.
        return jsonify({"ok": False, "error": "Unknown display"}), 404
    except Exception:
        log.exception("brightness set failed")
        return jsonify({"ok": False, "error": "Could not set brightness"}), 500
